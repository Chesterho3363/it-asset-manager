...
function flattenPage(page) {
  ...
  return {
    ...
    acquisitionDate: prop.date(p.AcquisitionDate), // ── 新增：取得日讀取 ──
...
function buildProperties(data) {
  ...
  if (data.acquisitionDate !== undefined) properties.AcquisitionDate = { date: data.acquisitionDate ? { start: data.acquisitionDate } : null }; // ── 新增：取得日寫入 ──
