...
function CategorySelector({ value, onChange, t }) {
  ...
  return (
    /* 🌟 改用 Flexbox Wrap，手機自動變上下兩排，絕對不跑版 */
    <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
      {options.map(o => (
        <button key={o.val} type="button" onClick={() => onChange(o.val)} style={{
          flex: "1 1 calc(50% - 0.5rem)", minWidth: "110px", /* 🌟 自動分配寬度 */
          padding: "0.6rem", background: value === o.val ? "var(--accent)" : "var(--bg-base)",
          border: `1px solid ${value === o.val ? "var(--accent)" : "var(--border)"}`,
          borderRadius: "8px", 
          color: value === o.val ? "var(--bg-base)" : "var(--text-secondary)",
          fontSize: "0.82rem", fontFamily: "var(--font-display)", cursor: "pointer",
  ...
  return (
    <div style={{ border: "1px solid var(--border)", borderRadius: "10px", overflow: "hidden" }}>
      <button type="button" onClick={() => setOpen(o => !o)} style={{
        ...
        color: "var(--text-secondary)", fontFamily: "var(--font-display)", fontSize: "0.8rem", fontWeight: 600,
      }}>
      ...
      {open && (
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem", padding: "1rem" }}>
          {fields.map(f => (
            <div key={f.key} style={{ flex: "1 1 130px" }}> {/* 🌟 自動伸縮 */}
...
export default function AssetForm({ onClose, onSuccess, editData = null }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 100, background: "rgba(0,0,0,0.6)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}
      ...
      <div className="animate-fade-in" style={{ background: "var(--bg-surface)", border: "1px solid var(--border)", borderRadius: "16px", width: "100%", maxWidth: "560px", maxHeight: "90vh", overflowY: "auto", boxShadow: "var(--shadow-lg)" }}>
        ...
        {/* Body */}
        <div style={{ padding: "1.25rem 1.5rem", display: "flex", flexDirection: "column", gap: "1rem", overflowX: "hidden" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "0.75rem" }}>
            ...
            <div style={{ flex: "1 1 140px" }}>
              ...
              <input style={inputStyle} value={form.model} onChange={set("model")} placeholder="MacBook Pro"
                onFocus={e => e.target.style.borderColor = "var(--border-focus)"}
                onBlur={e => e.target.style.borderColor = "var(--border)"} />
            </div>
            <div style={{ flex: "1 1 100%" }}> {/* 日期輸入框佔滿整行避免被擠壓 */}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem" }}>
              {[{ val: "available", zh: "✅ 可借用", en: "✅ Available" }, { val: "borrowed", zh: "⏳ 借出中", en: "⏳ Borrowed" }].map(o => (
                <button key={o.val} type="button" onClick={() => setForm(f => ({ ...f, status: o.val }))} style={{
                  ...
                  fontSize: "0.82rem", fontFamily: "var(--font-display)", cursor: "pointer", transition: "all 0.15s",
                }}>{t(o.zh, o.en)}</button>
              ))}
            </div>
          </div>
        ...
        {/* Footer */}
        <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid var(--border)", display: "flex", flexWrap: "wrap", gap: "0.75rem", position: "sticky", bottom: 0, background: "var(--bg-surface)" }}>
          {isEdit && (
            <button onClick={handleDelete} disabled={deleting} style={{
              padding: "0.6rem 1rem", background: "var(--danger-soft)", border: "1px solid var(--danger)",
              borderRadius: "8px", color: "var(--danger)", fontSize: "0.85rem", fontFamily: "var(--font-display)",
          ...
          <div style={{ flex: 1 }} />
          <button onClick={onClose} style={{ padding: "0.6rem 1.25rem", background: "var(--bg-elevated)", border: "1px solid var(--border)", borderRadius: "8px", color: "var(--text-secondary)", fontSize: "0.85rem", fontFamily: "var(--font-display)", cursor: "pointer" }}>
            {t("取消", "Cancel")}
          </button>
          <button onClick={handleSubmit} disabled={loading} style={{
            padding: "0.6rem 1.25rem", background: "var(--accent)", border: "none", borderRadius: "8px",
            color: "var(--bg-base)", fontSize: "0.85rem", fontFamily: "var(--font-display)",
